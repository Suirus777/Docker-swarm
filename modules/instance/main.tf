...
# SSH ключ для доступа к нодам из сервера где применяется код Terraform. 
# Важно ключи нужно создавать без пароля, Terraform не умеет работать с парольными ключами.
